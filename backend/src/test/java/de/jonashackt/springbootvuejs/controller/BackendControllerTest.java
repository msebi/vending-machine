...
public class BackendControllerTest {
	...
	public void addNewUserAndRetrieveItBack() {
		...
		Response responseCreate = given().header("Content-type", "application/json").and().body(norbertSiegmundJson)
		...
		User responseGet = given().pathParam("id", responseCreate.jsonPath().getString("id")).when()
	...
	public void user_api_should_give_http_404_not_found_when_user_not_present_in_db() {
		Long someId = 200L;
		given().pathParam("id", someId).when().get("/api/user/{id}").then().statusCode(HttpStatus.SC_NOT_FOUND);
	}
	...
	public void secured_api_should_give_http_200_when_authorized() {
		...
		given().auth().basic("sina", "miller").when().get("/api/secured").then().statusCode(HttpStatus.SC_OK)
